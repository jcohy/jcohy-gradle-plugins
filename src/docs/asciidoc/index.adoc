...
* jcohy-java-convention：可以应用于任何 Java 项目的一组插件，以提供一致的风格样式。请参考 <<jcohy-conventions>> 获取更多信息
* jcohy-java-checkstyle：一个 checkstyle 插件。集成 alibaba,sun,google,spring 和 jcohy（此项目自定义提供） 代码检查规范。请参考 <<checkstyles>>  获取更多信息
* jcohy-ali-oss: 用于向 ali oss 上传文件的插件. 请参考 <<jcohy-ali-oss>> 获取更多信息
...
buildscript {
	...
	dependencies {
		classpath("com.jcohy.gradle:jcohy-java-convention:${version}")
	}
}
...
include::alioss.adoc[leveloffset=+1]
