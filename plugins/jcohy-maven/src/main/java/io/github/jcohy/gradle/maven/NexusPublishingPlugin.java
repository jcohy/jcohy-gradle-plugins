...
public class NexusPublishingPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        extension.getRepositories().create("sonatype", repository -> {
            repository.getUsername().set(System.getenv("NEXUS_OSS_USER_NAME"));
            repository.getPassword().set(System.getenv("NEXUS_OSS_PASS_WORD"));
            repository.getNexusUrl().set(URI.create("https://oss.sonatype.org/service/local/staging/deploy/maven2"));
            repository.getSnapshotRepositoryUrl().set(URI.create("https://oss.sonatype.org/content/repositories/snapshots"));
        });
