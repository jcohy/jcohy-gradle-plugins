...
[IMPORTANT]
====
`closeAndReleaseSonatypeStagingRepository` 必须和 `publishToSonatype` 或 `publishMavenJavaPublicationToSonatypeRepository` 在同一个命名行中执行，否则会出现如下错误: No staging repository with name sonatype created.
====
